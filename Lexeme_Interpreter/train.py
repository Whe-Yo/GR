    ...
    def train(self) -> None:
        ...
        for epoch in range(1, num_epoch+1):
            ...
            for _, batch in enumerate(self.data_loader_train):
                ...
                logits = self.net(aud[:, self.uniform_len:, :], lxm_idx)
                ...
                # loss = self.criterion(logits.reshape(-1, logits.shape[-1]), lxm_idx[:, 1:].reshape(-1))
                a = logits.reshape(-1, logits.shape[-1])
                b = lxm_idx[:, 1:].reshape(-1)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        # path_config = "Config/Trinity/config_vqvae.json5"

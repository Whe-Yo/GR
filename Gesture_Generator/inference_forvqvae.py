...
from Data_Preprocessing.Utils.utils import inv_standardize, expmap_to_euler, resample_motion, split_dataset, sixd_to_euler
...
from Gesture_Lexicon.lexicon_VQVAE import predict_lexeme
...
def get_args_parser():
    ...
    parser.add_argument('--data_dir', type=str, default='../data/Trinity/Source/Test_Data')
    parser.add_argument('--name_file', type=str, default='TestSeq001')
    parser.add_argument('--random_cutsample_sec', type=str, default=None) # None : whole time
    parser.add_argument('--checkpoint_path', type=str, default='./Training/Trinity/_RNN_RVQ/Checkpoints/trained_model.pth')
    parser.add_argument('--checkpoint_config', type=str, default='./Training/Trinity/_RNN_RVQ/config.json5')
    parser.add_argument('--lxc_checkpoint_path', type=str,
                        default='../Gesture_Lexicon/Training/Trinity/_VQVAE_conv_20240609_112415/Checkpoints/trained_model.pth')
    parser.add_argument('--lxc_checkpoint_config', type=str, default='../Gesture_Lexicon/Training/Trinity/_VQVAE_conv_20240609_112415/config.json5')
    parser.add_argument('--device', type=str, default='cuda:0')
    parser.add_argument('--save_dir', type=str, default='../data/Trinity/Source/Test_Data')
...
class Inference:
    def __init__(self,
        ...
        with open(path_lxc_config, "r") as f:
            self.lxc_config = json5.load(f)
            self.batch_size = self.lxc_config['batch_size']
        ...
        self.uniform_len = int(self.config_data_preprocessing['fps'] * self.config_data_preprocessing['onset_bounds'][1])
        ...
        self.step = self.config_data_preprocessing["step"]
        ...
        self.net_vqvae = vqvae_net(self.lxc_config, self.config_data_preprocessing)

        # # RNN
        # self.net.load_state_dict(torch.load(path_pretrained, map_location=torch.device("cpu")))
        # # VQVAE
        checkpoint = torch.load(self.path_lxc_net)
        self.net_vqvae.load_state_dict(checkpoint['model_state_dict'])

        # self.net.to(self.device)
        self.net_vqvae.to(self.device)
    ...
    def sample(self, dir_data: str, name_file: str, dir_save: str = "./"):
        ...
        preprocessor = Preprocessor(path_config=os.path.join(dir_save, "config.json5"), isInference=True, random_cutsample_sec=args.random_cutsample_sec)
        ...
        num_blocks = len(np.load(os.path.join(dir_save, "Features", "Data_Len_Uniform", name_file + "_index.npy")))  # len_block = 10
        uniform_len = int(self.uniform_len)
        ...
        _, _, _ = split_dataset(dir_audio_feat=os.path.join(dir_save, "Features", "Audio_Feature"),
                                ...
                                # uniform_len=uniform_len, num_blocks_per_clip=self.num_blocks, step=self.step,
        ...
        #                self.path_lxc_net, self.path_lxc_config, self.device, True)
        ...
        # # VQVAE
        infer_res = infer_train_forvqvae(batch, self.device, self.net_vqvae, uniform_len, num_blocks, self.config["network"]["name"], self.batch_size)
        ...
        # num_rotation = 6
        #
        # f, s, t = motion_gt.shape
        # motion_gt = motion_gt.reshape(-1, num_rotation)
        # motion_pred = motion_pred.reshape(-1, num_rotation)
        #
        # motion_gt = rotation_6d_to_matrix(motion_gt)
        # # input = smoothing(input, is_window=False, order='zxy')
        # motion_gt = Rotation.from_matrix(motion_gt).as_quat()
        # motion_gt = motion_gt.reshape(f, s, t)
        #
        #
        # motion_pred = rotation_6d_to_matrix(motion_pred)
        # # input = smoothing(input, is_window=False, order='zxy')
        # motion_pred = motion_pred.reshape(f, s, t)
        #
        # #
        ...
        for i in range(batch_size):
            ...
            motion_gt_euler.append(sixd_to_euler(data_frame_gt, self.rotation_order).to_numpy())
            ...
            motion_pred_euler.append(sixd_to_euler(data_frame_pred, self.rotation_order).to_numpy())
        ...
        fgd_value = calculate_fgd(mu1, sigma1, mu2, sigma2)
        print("FGD:", fgd_value)

        mse_value = np.mean((motion_gt - motion_pred) ** 2)
        print("MSE:", mse_value)
...
if __name__ == "__main__":
    ...
    if os.path.exists(os.path.join(args.data_dir, args.name_file+'.wav')) and os.path.exists(os.path.join(args.data_dir, args.name_file+'.bvh')):

...
def get_args_parser():
    ...
    parser.add_argument('--checkpoint_path', type=str, default='./Training/Trinity/_RNN_RVQ_VQVAE/Checkpoints/trained_model.pth')
    parser.add_argument('--checkpoint_config', type=str, default='./Training/Trinity/_RNN_RVQ_VQVAE/config.json5')
    parser.add_argument('--lxc_checkpoint_path', type=str,
                        default='../Gesture_Lexicon/Training/Trinity/_RVQ_conv_qqloss2/Checkpoints/trained_model.pth')
    parser.add_argument('--lxc_checkpoint_config', type=str, default='../Gesture_Lexicon/Training/Trinity/_RVQ_conv_qqloss2/config.json5')
...
class Inference:
    ...
    def sample(self, dir_data: str, name_file: str, dir_save: str = "./"):
        ...
        _, _, _ = split_dataset(dir_audio_feat=os.path.join(dir_save, "Features", "Audio_Feature"),
                                ...
                                uniform_len=uniform_len, num_blocks_per_clip=num_blocks, step=num_blocks,
                                # means batch size is 1
        ...
        predict_lexeme(os.path.join(dir_save, f"{dataset_type}.npz"),
                       ...
                       self.device, save=True, isinference=True)
        ...
        for row in range(indexes_selected.shape[0]):
            for col in range(indexes_selected.shape[1]):
                ...
                if (key not in index_motion_gt_pair.keys()) and (key not in index_motion_pred_pair.keys()):
                    if col == 0:
                        index_motion_gt_pair[key] = motion_gt_euler[row][
                                                    col * self.uniform_len: (col + 1) * self.uniform_len, :]
                        index_motion_pred_pair[key] = motion_pred_euler[row][
                                                      col * self.uniform_len: (col + 1) * self.uniform_len, :]
                    else:
                        index_motion_gt_pair[key] = motion_gt_euler[row][
                                                    col * self.uniform_len - 1: (col + 1) * self.uniform_len, :]
                        index_motion_pred_pair[key] = motion_pred_euler[row][
                                                      col * self.uniform_len - 1: (col + 1) * self.uniform_len, :]
                else:
                    raise ValueError

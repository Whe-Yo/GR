...
def get_args_parser():
    ...
    parser.add_argument('--name_file', type=str, default='Recording_001')
    parser.add_argument('--random_cutsample_sec', type=str, default=None) # None : whole time
    parser.add_argument('--checkpoint_path', type=str, default='./Training/Trinity/_RNN_20240213_182836/Checkpoints/trained_model.pth')
    parser.add_argument('--checkpoint_config', type=str, default='./Training/Trinity/_RNN_20240213_182836/config.json5')
    parser.add_argument('--lxc_checkpoint_path', type=str,
                        default='../Gesture_Lexicon/Training/Trinity/_VQVAE_gcn_20240214_182125/Checkpoints/trained_model.pth')
    parser.add_argument('--lxc_checkpoint_config', type=str, default='../Gesture_Lexicon/Training/Trinity/_VQVAE_gcn_20240214_182125/config.json5')
...
class Inference:
    def __init__(self,
        ...
        # # VQVAE
        # self.net_vqvae.load_state_dict(torch.load(path_pretrained, map_location=torch.device("cpu")))
    ...
    def sample(self, dir_data: str, name_file: str, dir_save: str = "./"):
        ...
        lxm_idx = batch["lexeme_index"]  # [N, B]

        for cluster in range(50):
            if torch.any(lxm_idx == cluster):

                infer_res = infer_train_forlexeme(batch, self.device, self.net_vqvae, uniform_len, num_blocks, self.config["network"]["name"], cluster)
                # infer_res = infer_train(batch, self.device, self.net_vqvae, uniform_len, num_blocks, self.config["network"]["name"])

                motion_gt = infer_res[0].detach().cpu().numpy()  # [N, L, D]
                motion_pred = infer_res[1].detach().cpu().numpy()

                # num_blocks = 10  # for lxm test
                ...
                for i in range(batch_size):
                    ...
                    with open(os.path.join(dir_save, f"{name_file}_{cluster}_gt.bvh"), "w") as f:
                        writer.write(inv_clips[0], f, framerate=self.fps)

                    with open(os.path.join(dir_save, f"{name_file}_{cluster}.bvh"), "w") as f:
                ...
                for i in range(batch_size):
                    path_bvh = [
                        os.path.join(dir_save, f"{name_file}_{cluster}_gt.bvh"),
                        os.path.join(dir_save, f"{name_file}_{cluster}.bvh")
                    ]

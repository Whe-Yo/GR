...
class MotionGenerator_RNN(nn.Module):
    def __init__(self,
        ...
        self.motion_decoder = GRUDecoder(aud_embed_dim, lxm_dim, rnn_hid_dim, rnn_out_dim, rnn_depth)
        self.cell_state_init = CellStateInitializer(lxm_dim, rnn_hid_dim, rnn_depth)
    ...
    def forward(self, aud, lxm):
        ...
        cell_state = self.cell_state_init(lxm[:, :, 0])
        ...
        for b_idx in range(B-2):
            ...
            for f in range(BL):  # f: frame
                ...
                mo_pred, cell_state = self.motion_decoder(cur_aud_embed, cur_lxm, cell_state)
...
class GRUDecoder(nn.Module):
    def __init__(self, aud_embed_dim, lxm_dim, hidden_dim, out_dim, depth):
        super().__init__()

        in_dim = aud_embed_dim + lxm_dim
    ...
    def forward(self, aud_embed, lxm, cell_state):
        """
        aud_embed, lxm: [N, D]
        cell_state: [P, N, D]
        """
        in_embed = F.gelu(self.embed_in(torch.cat([aud_embed, lxm], dim=-1)))
        cell_out, cell_state = self.gru(
            torch.cat([in_embed, aud_embed, lxm], dim=-1).unsqueeze(1), cell_state  # add an input embedding (redundancy) into the gru input
        )
...
class CellStateInitializer(nn.Module):
    ...
    def forward(self, lxm):
        """
        lxm: [N, D]
        """
        x = F.gelu(self.layer0(lxm))
        x = F.gelu(self.layer1(x))
        x = self.layer2(x)
...
if __name__ == "__main__":
    ...
    # model = CellStateInitializer(96, 1024, 2).to(device)
    #
    # x = torch.rand([5, 96]).to(device)  # [N, D]
    # y = model(x)
    #
    # print(y.shape)
    ...
    # model = GRUDecoder(64, 96, 1024, 48, 2).to(device)
    #
    # x = torch.rand([5, 64]).to(device)  # [N, D]
    # x1 = torch.rand([5, 96]).to(device)  # [N, D]
    # x2 = torch.rand([2, 5, 1024]).to(device)  # [N, D]
    # y, y1 = model(x, x1, x2)
    #
    # print(y.shape)
    # print(y1.shape)
    ...
    # x1 = torch.rand([5, 96, 10]).to(device)  # [N, D, L]
    #
    # y = model(x, x1)
    #
    # print(y.shape)

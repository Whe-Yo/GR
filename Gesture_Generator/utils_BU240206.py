...
from Gesture_Lexicon.model import VQVAE_gcn, VQVAE_conv, RVQ_gcn, RVQ_conv, Transformer
...
__all__ = ["infer_train", "infer_train_forvqvae", "infer_train_forkmeans", "infer_train_forlexeme", "initialize_net", "vqvae_net"]
...
def infer_train(batch, device, net, uniform_len, num_blocks, name_net):
    if name_net == "RNN":
        ...
        if lxm.shape[0] == aud.shape[1]//uniform_len:
            lxm = lxm.view(lxm.shape[2], lxm.shape[0], lxm.shape[1])
...
def infer_train_forlexeme(batch, device, net, uniform_len, num_blocks, name_net, cluster):
    ...
    if name_net == "RNN":
        ...
        x_lexeme = x_lexeme.permute((0, 2, 1)).contiguous()
        x_lexeme = x_lexeme.squeeze(0)
        z_q, mo_hat, vq_loss = net.forward_forlxm(x_lexeme)

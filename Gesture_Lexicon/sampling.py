...
class Inference:
    def __init__(self, 
                 ...
                 device : str, 
                 path_config_train : str,
                 isinference : bool = False) -> None:
        ...
        checkpoint = torch.load(path_pretrained_net)
    ...
    def infer(self) -> np.ndarray:
        ...
        with torch.no_grad():
            latent_codes = []
            latent_codes_indices = []
            for batch in self.data_loader:
                motion_block = batch['motion'].to(self.device)  # batch_size X dim_feat X time.
            
                if self.config['network']['name'] in ['Conv1d', 'Transformer', 'VQVAE_gcn', 'VQVAE_conv', 'RVQ_gcn', 'RVQ_conv']:
                    latent_code, motion_block_hat, indices, _, _ = self.net(motion_block)
                else:
                    raise NotImplementedError
                ...
                latent_codes_indices.append(indices)

                # latent_code = latent_code.detach().cpu().numpy()
                # if latent_code.shape[0] < self.dataset.index.shape[1]:
                #     latent_code = np.concatenate(np.split(latent_code, latent_code.shape[0], axis=0), axis=2)
                # elif latent_code.shape[0] >= self.dataset.index.shape[1]:
                #     latent_code = np.concatenate(np.split(latent_code, self.dataset.index.shape[1], axis=0), axis=2)
                # # latent_code = np.concatenate(np.split(latent_code, latent_code.shape[0], axis=0), axis=2)
                # latent_code = latent_code.reshape(latent_code.shape[0], latent_code.shape[2], latent_code.shape[1])
                # # latent_code = np.transpose(latent_code, (0, 2, 1))  # num_clips X num_blocks X dim_feat.
                # latent_codes.append(latent_code)
                #
                # indices = indices.detach().cpu().numpy()
                # # indices = np.concatenate(np.split(indices, indices.shape[0], axis=0), axis=1)
                # if latent_code.shape[0] < self.dataset.index.shape[1]:
                #     indices = np.concatenate(np.split(indices, latent_code.shape[0], axis=0), axis=1)
                # elif latent_code.shape[0] >= self.dataset.index.shape[1]:
                #     indices = np.concatenate(np.split(indices, self.dataset.index.shape[1], axis=0), axis=1)
                # latent_codes_indices.append(indices)
        ...
        # if latent_code.shape[0] < self.dataset.index.shape[1]:
        #     latent_codes = np.concatenate(latent_codes, axis=1)  # num_clips X num_blocks X dim_feat.
        #     latent_codes_indices = np.concatenate(latent_codes_indices, axis=0)
        # elif latent_code.shape[0] >= self.dataset.index.shape[1]:
        ...
        latent_codes = np.concatenate(np.split(latent_codes, self.dataset.index.shape[1], axis=0), axis=2)
        latent_codes = latent_codes.reshape(latent_codes.shape[0], latent_codes.shape[2], latent_codes.shape[1])
        # latent_codes = np.transpose(latent_codes, (0, 2, 1))  # num_clips X num_blocks X dim_feat.

        latent_codes_indices = np.concatenate(np.split(latent_codes_indices, self.dataset.index.shape[1], axis=0), axis=1)

        # return latent_codes
        return latent_codes, latent_codes_indices
    ...
    def infer(self) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            for batch in self.data_loader:
                ...
                if self.config['network']['name'] in ['Conv1d', 'Transformer', 'VQVAE_gcn', 'VQVAE_conv', 'RVQ_gcn', 'RVQ_conv']:
                    latent_code, motion_block_hat, indices, _, _, _ = self.net(motion_block)
                else:
                    raise NotImplementedError
        ...
        latent_codes = np.concatenate(latent_codes, axis=0)  # num_clips X num_blocks X dim_feat.
        latent_codes_indices = np.concatenate(latent_codes_indices, axis=0)  # num_clips X num_blocks.

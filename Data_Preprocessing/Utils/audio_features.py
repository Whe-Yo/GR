...
__all__ = ["load_audio", "extract_melspec", "detect_onset", "prepare_audio_feature",
           "load_audio_fromtrinity", "detect_onset_fromtrinity"]
...
def load_audio_fromtrinity(dir_audio: str, names_file: List[str], dir_save: str, audiostarttimes: List,
               sr: int = 48000, normalize_loudness: bool = True, save: bool = False) -> List[np.ndarray]:
    res = []

    audiostarttimes_sr = audiostarttimes * sr

    for i, n in enumerate(tqdm(names_file)):
        path_audio = os.path.join(dir_audio, n + ".wav")
        audio, _ = librosa.load(path_audio, sr=sr)
        audio = audio[int(audiostarttimes_sr[i]):]
...
def extract_melspec(dir_loaded_audio: str, names_file: List[str], dir_save: str,
    ...
    for name in names_file:
        ...
        outfile_high = os.path.join(dir_save, name+'_high.npy')
        outfile_low = os.path.join(dir_save, name+'_low.npy')
        ...
        # high level feature, MFCC & interpolation
        mfcc = librosa.feature.mfcc(S=librosa.power_to_db(mel.T, ref=np.max), sr=sr, n_mfcc=13).T  # [L, D]
        mfcc = griddata(
            np.arange(len(mfcc)),
            mfcc,
            sr / mel_hop_len / fps * np.arange(mo_len_pred),
        ...
        # low level feature, spectral contrast & interpolation
        spectral_contrast = librosa.feature.spectral_contrast(S=librosa.power_to_db(mel.T, ref=np.max), sr=sr).T  # [L, D]

        spectral_contrast = griddata(
            np.arange(len(spectral_contrast)),
            spectral_contrast,
            sr / mel_hop_len / fps * np.arange(mo_len_pred),
            'linear'
        ).astype(np.float32)
        ...
        if save:
            np.save(outfile, mel)
            np.save(outfile_high, mfcc)
            np.save(outfile_low, spectral_contrast)
...
def detect_onset(dir_loaded_audio: str, dir_loaded_mel: str,  names_file: List[str], dir_save: str,
    ...
    for n in names_file:
        ...
        audio_high = np.load(os.path.join(dir_loaded_mel, n+"_high.npy"))
        audio_low = np.load(os.path.join(dir_loaded_mel, n+"_low.npy"))
        ...
        # low level audio feature에서 onset 탐색
        spectral_novelty_low = np.diff(audio_low, axis=1)
        spectral_novelty_low = np.mean(spectral_novelty, axis=0)
...
def detect_onset_fromtrinity(dir_loaded_audio: str, names_file: List[str], dir_save: str, dir_text: str,
                 fps: int, sr: int, bounds: List[float], save: bool = False) -> List[np.ndarray]:
    """
    Detect audio onset.
    (Optional)Save detected onset(shape=(num_onsets,), unit is `frame`) to destination path.
    ...
    for n in tqdm(names_file):
        audio = np.load(os.path.join(dir_loaded_audio, n + ".npy"))

        # # Trinity 2 Beat 시간 읽기

        text = open(os.path.join(dir_text, n + ".phrase"), 'r')
        ...
        for line in text:
            if 'TimeOffset' in line or 'start' in line:
                continue

            line = line.replace('\n', '')
            line = line.split(' ')

            text_min_times.append(float(line[0]))
            text_max_times.append(float(line[1]))
            text_contexts.append(line[2])

        text_min_times = np.array(text_min_times)
        text_max_times = np.array(text_max_times)
        text_contexts = np.array(text_contexts)
        ...
        #                                                   pre_avg=1, post_avg=1, pre_max=1, post_max=post_max,
        #                                                   delta=0.0, hop_length=hop_length, units='time')
        onset_times_spectral = np.sort(np.append(text_min_times, text_max_times))
        onsets_spectral = librosa.time_to_frames(onset_times_spectral, sr=sr, hop_length=hop_length).astype(int)

        # # # region Filter onset. 새 pseudo onset 삽입
        #
        # while True:
        #     onsets_selected = []
        #     empty_counter = 0

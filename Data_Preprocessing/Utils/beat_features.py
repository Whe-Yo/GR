...
__all__ = ["align_mel_with_motion", "uniform_data_fragment_length_tsm",
           "align_mel_with_motion_randomcutsample", "uniform_data_fragment_length_tsm_fromtrinity"]
...
def align_mel_with_motion_randomcutsample(dir_mel: str, dir_motion: str, names_file: List[str],
                           dir_save: str, save: bool = False, fps: int = None, random_cutsample_sec: int = None) -> Tuple[List[np.ndarray], List[pd.DataFrame]]:
    res_mel = []
    res_motion = []
    random_cutsample_fps = random_cutsample_sec * fps
    ...
    def process(suffix: str = ""):
        for n in tqdm(names_file):
            ...
            random_startframe = random.randrange(0, (motion.shape[0]-random_cutsample_fps)+1)
            mel = mel[random_startframe:random_startframe+random_cutsample_fps, :]
            motion = motion.iloc[random_startframe:random_startframe+random_cutsample_fps, :]
...
def uniform_data_fragment_length_tsm_fromtrinity(dir_mel_motion_aligned: str, dir_onset: str, dir_wav: str,
                                     names_file: List[str], dir_save: str,
                                     fps: int, sr: int, rotation_order: str, uniform_len: int,
                                     save: bool = False) -> Tuple[List[np.ndarray], List[np.ndarray], List[pd.DataFrame]]:
        ...
        for n in tqdm(names_file):
            ...
            for idx, s in enumerate(scopes):
                if idx == 0:
                    ...
                    motions_resampled.append(
                        resample_motion(motion.iloc[s[0]: s[1] + 1, :], ticks, target_ticks, rotation_order).to_numpy()[1:, :])

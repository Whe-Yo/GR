...
def load_from_io(f, fn='', insert_T_pose:bool=False, ignore_root_offset=True, max_frames=None):
    ...
    for i,ch in enumerate(channels):
        ...
        if not ch[1][0] == '':
            if 'Recording' not in f.name and joints[i] == 'Hips':
                # skeleton이 y rotation으로 누워있는 경우 하드코딩
                # Trinity GENEA 2020의 경우 무시
                # Trinity 2의 경우 euler y rotation 수정
                if np.any(joint_values[1:10, 4] > 80):
                    joint_values[:, 4] = joint_values[:, 4] - 90
                elif np.any(joint_values[1:10, 4] < -80):
                    joint_values[:, 4] = joint_values[:, 4] + 90

            rot = R.from_euler(ch[1][0], joint_values[:, ch[1][1]], degrees=True)
            data._joint_rotation[:,i] = rot.as_quat()
            # data._joint_rotation[:,i] = joint_values[:, ch[1][1]]

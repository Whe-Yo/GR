...
__all__ = ["extract_bvh_motion", "transfer_to_facing_coordinate", "make_motion_template", "prepare_motion_feature",
           "transfer_to_facing_coordinate_fromtrinity"]
...
def transfer_to_facing_coordinate_fromtrinity(dir_motion: str, names_file: List[str], dir_save: str,
                                  rotation_order: str, motionstarttimes: List):
    for i, name in enumerate(tqdm(names_file)):
        motion_data = bvh_load(os.path.join(dir_motion, name + ".bvh"))  # get joint information

        motion_data._joint_orientation = motion_data.joint_orientation[int(motionstarttimes[i] * motion_data.fps):]
        motion_data._joint_position = motion_data.joint_position[int(motionstarttimes[i] * motion_data.fps):]
        motion_data._joint_rotation = motion_data.joint_rotation[int(motionstarttimes[i] * motion_data.fps):]
        motion_data._joint_rotation_euler = motion_data.joint_rotation_euler[int(motionstarttimes[i] * motion_data.fps):]
        motion_data._joint_translation = motion_data.joint_translation[int(motionstarttimes[i] * motion_data.fps):]
        motion_data._num_frames = motion_data.joint_rotation.shape[0]
...
def prepare_motion_feature(dir_data_len_uniform: str, names_file: List[str], dir_save: str,
    ...
    def process(suffix: str = ""):
        for n in names_file:
            ...
            # new_motion = euler_to_expmap(motion_filtered, rotation_order) # expmap
            new_motion = euler_to_6d(motion_filtered, rotation_order) # 6d
